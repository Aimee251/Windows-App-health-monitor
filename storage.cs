...
public sealed class MetricsStore
{
    ...
    public IReadOnlyList<MetricSample> History(int processId) => 
        _byProcess.TryGetValue(processId, out var buf) ? buf.Items().ToList() : Array.Empty<MetricSample>();

    // the most recent sample for every process still alive (for the table)
    public IReadOnlyList<MetricSample> LatestPerProcess() => 
        _byProcess.Values.Select(b => b.Items().LastOrDefault()).Where(s => s is not null).ToList()!;
}